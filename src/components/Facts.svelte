...
<section class="py-20 relative">
  <div class="max-w-7xl mx-auto px-4">
    <h2 class="text-4xl md:text-5xl font-bold text-center mb-16 text-glow-strong">Fatos Fascinantes</h2>
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
      {#each facts as fact, i}
        <div
          class="fact-card bg-gradient-to-br from-event-horizon to-dark-matter p-8 rounded-lg border border-accretion/30 hover:border-accretion/60 transition-all hover:scale-105 hover:shadow-xl hover:shadow-accretion/20"
          key={i}
        >
          <div class="text-4xl mb-4">{fact.icon}</div>
          <h3 class="text-xl font-bold mb-3 text-accretion">{fact.title}</h3>
          <p class="text-gray-300 text-sm leading-relaxed">{fact.description}</p>
        </div>
      {/each}
    </div>
  </div>
